...
from .function import normal, coral
from . import net
import numpy as np
import time
...
def feat_extractor(vgg, content, style):
    ...
    style_fs = [enc_1(style), enc_2(enc_1(style)), enc_3(
        enc_2(enc_1(style))), Style4_1, Style5_1]
...
def get_parser():
    ...
    parser.add_argument('--decoder_path', type=str,
                        default='models/decoder_iter_160000.pth')
    parser.add_argument('--transform_path', type=str,
                        default='models/ma_module_iter_160000.pth')
...
def style_transfer_main(args):
    ...
    if args.style:
        ...
        if len(style_paths) == 1:
            ...
            weights = [int(i)
                       for i in args.style_interpolation_weights.split(',')]
    ...
    for content_path in content_paths:
        if do_interpolation:  # one content image, N style image
            ...
            output_name = '{:s}/{:s}_interpolation_{:s}'.format(
                args.output_dir, splitext(basename(content_path))[0], save_ext)
            ...
            for style_path in style_paths:
                ...
                with torch.no_grad():
                    ...
                    output = style_transfer(
                        vgg, decoder, ma_module, content, style, alpha)

                output = output.cpu()

                output_name = '{:s}/{:s}_stylized_{:s}{:s}'.format(
                    args.output_dir, splitext(basename(content_path))[0],
